...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'get-url') {
      ...
      // Build a deterministic, filesystem-safe storage path using canonical format
      // Sanitize filename: replace spaces with dashes, remove problematic URL characters, lowercase
      ...
      // Use canonical format: {report_id}-{sanitized-filename}
      const path = `${report_id}-${safeName}`;
